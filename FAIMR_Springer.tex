% =====================================================================
% FAIMR: Springer LNCS/LNNS format
% Converted from IEEEtran for submission to IntelliSys 2026
% (Lecture Notes in Networks and Systems, Springer)
% =====================================================================

\documentclass{llncs}

% --- Packages retained from original (IEEEtran-specific packages removed) ---
% Removed: \usepackage{cite}      -- conflicts with splncs04 citation style
% Removed: \usepackage{textcomp}  -- IEEEtran utility, not needed in llncs
% Removed: \usepackage{amsthm}    -- llncs defines its own theorem environments;
%                                    loading amsthm causes "already defined" errors
% Removed: \newtheorem{theorem}, {lemma}, {definition}
%                                 -- built-in to llncs; re-declaring causes errors
% Note on proposition: llncs 2022+ defines \proposition via \spnewtheorem.
%   If your llncs.cls does NOT define it, uncomment the line below:
...
\usepackage{booktabs}
\usepackage{multirow}
\usepackage{hyperref}   % Compatible with llncs; Springer asks to remove for
                        % camera-ready, but fine for draft/review
\usepackage{url}

% Custom theorem environment not present in llncs
\newtheorem{empiricalobs}{Empirical Observation}

\begin{document}

\title{FAIMR: A Fairness-Aware and Explainable Multi-Signal Framework for Automated Resume Ranking}

\author{Arnav Gupta}

\institute{Department of Computer Science (AIML), Vishwakarma Institute of Technology, Pune, India\\
\email{arnav.g1010@gmail.com}}

\maketitle

\begin{abstract}
Automated Resume Screening (ARS) systems that rely on a single similarity signal suffer from keyword dilution, demographic bias, and decision opacity. We propose FAIMR (Fairness-Aware Explainable Multi-Signal Resume Re-ranking), a framework that fuses Sentence-BERT contextual embeddings, TF-IDF lexical similarity, and deterministic skill-graph coverage into a gradient-boosted Learning-to-Rank ensemble. We provide a theoretical analysis showing that the three signal families occupy complementary regions of the representation space, and that their non-linear fusion via gradient boosting captures feature interactions inaccessible to any single representation. A post-hoc Fairness-Constrained Re-ranking (FCR) algorithm enforces the four-fifths rule of Adverse Impact while provably minimizing rank displacement, and a counterfactual explainer identifies the minimal skill acquisition path to invert a rejection via greedy submodular optimization with formal approximation guarantees. Evaluation on 26,020 domain-stratified resume--job pairs with verified feature--label independence ($r_{\mathrm{pb}}{=}0.14$) shows that FAIMR achieves an AUC of $0.719 \pm 0.010$, outperforming BM25 ($0.604$), Doc2Vec ($0.596$), BERT Cross-Encoder ($0.585$), MPNet-SBERT ($0.656$), and Jaccard Skill Matching ($0.581$), with each signal increment confirmed statistically significant ($p < 0.002$). The FCR module achieves an Adverse Impact Ratio of $0.979$ with zero rank displacement on unbiased data, and a stress test under synthetic demographic skew confirms it restores AIR~$\geq 0.8$ at all skew levels. The counterfactual generator produces explanations for 8,080 rejected candidates, averaging $1.19$ skills with mean latency $1.14$\,ms (median $0.79$\,ms) and $100\%$ greedy-optimal match rate.
\end{abstract}

\keywords{Learning-to-Rank \and Algorithmic Fairness \and Natural Language Processing \and Resume Screening \and Explainable AI \and Counterfactual Reasoning}
...
\end{equation}
where $f(t,d)$ is term frequency. While effective for exact matches, BM25 assigns zero score to synonymous terms, producing false negatives. Singh~\textit{et al.}~\cite{singh2020automated} document this limitation in first-generation applicant tracking systems.

Le and Mikolov~\cite{le2014doc2vec} introduced Doc2Vec (Paragraph Vectors), learning fixed-length document representations via distributed memory (PV-DM) and distributed bag-of-words (PV-DBOW) objectives. While an advance over bag-of-words, Doc2Vec lacks attention mechanisms and produces context-\textit{independent} embeddings.

\subsection{Neural Bi-Encoders and Cross-Encoders}

The Transformer architecture~\cite{vaswani2017attention} and BERT~\cite{devlin2018bert} enabled context-dependent embeddings. Reimers and Gurevych~\cite{reimers2019sbert} proposed SBERT, a Siamese bi-encoder producing sentence embeddings for efficient pairwise comparison. Song~\textit{et al.}~\cite{song2020mpnet} introduced MPNet, combining masked and permuted language modeling, achieving state-of-the-art results on sentence similarity benchmarks.
...
LTR methods provide a framework for combining heterogeneous relevance signals. Burges~\textit{et al.}~\cite{burges2005learning} established listwise and pairwise LTR objectives. Chen and Guestrin~\cite{chen2016xgboost} introduced XGBoost, a scalable gradient-boosted tree ensemble that learns non-linear feature interactions through iterative functional gradient descent. The key theoretical advantage of tree-based LTR over linear combination (weighted averaging) is the ability to learn \textit{conjunctive rules}: ``IF $S_\text{sem} > 0.8$ AND $C_\text{skill} < 0.2$ THEN irrelevant,'' which captures the keyword dilution failure mode directly.

\subsection{Algorithmic Fairness in Ranking}

Dwork~\textit{et al.}~\cite{dwork2012fairness} formalized individual and group fairness. Hardt~\textit{et al.}~\cite{hardt2016equality} contributed Equalized Odds. In ranking, Zehlike~\textit{et al.}~\cite{zehlike2017fa} proposed FA*IR with statistical representation guarantees. Foulds~\textit{et al.}~\cite{foulds2020intersectional} extended fairness to intersectional subgroups. Buolamwini and Gebru~\cite{buolamwini2018gender} demonstrated significant disparities in commercial ML systems.
...
\subsection{Fairness-Constrained Re-ranking: Correctness and Optimality}\label{sec:fcr_theory}
...
\subsection{Counterfactual Optimality Guarantee}\label{sec:cf_theory}
...
\subsection{Phase 3: Fairness-Constrained Re-ranking}\label{sec:fcr_method}
...
% Note: table* (double-column span) is not applicable in llncs single-column
% format. Converted to table with \small and \resizebox for width control.
\begin{table}[t]
\centering
...
\begin{table}[t]
\centering
\caption{FCR stress test under synthetic demographic skew ($n{=}26{,}020$, selection cutoff $k{=}0.2n$). \textit{Skew} is the demographic score penalty applied to the minority group as a fraction of the inter-quartile score range; it is varied independently of $k$. FCR restores AIR~$\geq 0.8$ at all skew levels.}
\label{tab:fcr_stress}
{\small
\begin{tabular}{@{}rcccrr@{}}
\toprule
Skew & AIR\textsubscript{b} & AIR\textsubscript{a} & 4/5 & Swaps & $\mathcal{D}$ \\
\midrule
0\% & .966 & .966 & \checkmark & 0 & 0 \\
5\% & .457 & .800 & \checkmark & 679 & 77 \\
10\% & .178 & .800 & \checkmark & 1,522 & 456 \\
15\% & .046 & .800 & \checkmark & 2,073 & 1,001 \\
20\% & .004 & .800 & \checkmark & 2,280 & 1,463 \\
25\% & .000 & .800 & \checkmark & 2,300 & 1,704 \\
30\% & .000 & .800 & \checkmark & 2,302 & 1,782 \\
\bottomrule
\end{tabular}}
\end{table}

Three observations emerge. First, FCR \textit{successfully restores} AIR~$\geq 0.8$ at every skew level, validating the algorithm's correctness guarantee (Theorem~\ref{thm:fcr}). Even at 30\% skew, where the pre-FCR AIR collapses to $0.000$ (complete demographic exclusion), FCR restores compliance through 2,302 swaps.

Second, the results reveal a \textit{fairness--displacement trade-off}. At moderate skew (5\%), correction requires only 679 swaps with a mean displacement of 77 ranks per candidate---a negligible perturbation relative to the pool of 26,020. At extreme skew (30\%), displacement rises to 1,782 ranks, reflecting the fundamental cost of correcting severe systemic bias. This trade-off is intrinsic to any post-processing fairness intervention and quantifies the ``price of fairness''~\cite{dwork2012fairness}.

Third, the number of swaps plateaus near 2,300 for skews $\geq 20\%$, indicating that FCR exhausts the available minority candidates near the cutoff boundary. This ceiling effect is consistent with the theoretical bound in Theorem~\ref{thm:fcr}: the algorithm terminates when no further eligible candidates exist for swapping.
...
\begin{table}[t]
\centering
\caption{Counterfactual explanation statistics (full rejected set). Optimality verification was performed on a stratified random sample of 100 candidates selected across the 5 test folds (20 per fold), covering the full range of $|\Delta|$ values observed in the dataset; all remaining instances with $|\Delta| \leq 8$ were also verified by brute force.}
...
Metric & Value \\
\midrule
Rejected candidates & 9,023 \\
With skill deficiency & 8,080 \\
Mean $|\delta^*|$ (skills) & 1.19 \\
Median / Std / Max $|\delta^*|$ & 1.0 / 0.42 / 3 \\
Mean / Median latency & 1.14 / 0.79 ms \\
Greedy = Optimal & 100\% (5,601/5,601) \\
\bottomrule
\end{tabular}}
\end{table}

Table~\ref{tab:counterfactual} reports counterfactual explanation statistics evaluated on the \textit{entire} rejected candidate set ($n = 8{,}080$ candidates with non-empty skill deficiencies), not a subsample. The mean $|\delta^*| = 1.19$ indicates that most rejections pivot on a \textit{single} missing skill (median $= 1.0$), with a maximum of only 3 skills required. This provides highly actionable feedback (\textit{e.g.}, ``Adding \textit{Docker} would move you from rank~\#7 to rank~\#3''). The low variance ($\sigma = 0.42$) confirms that explanation complexity is consistently bounded.

The $100\%$ greedy-optimal match rate across all 5,601 brute-force-verifiable instances ($|\Delta| \leq 8$) validates the submodularity assumption (Empirical Observation~\ref{prop:submod}): the greedy algorithm finds the minimum-cardinality solution in \textit{every} verified case. The median latency of $0.79$\,ms enables real-time deployment in interactive recruitment systems.
...
\textbf{On inferred demographics.} Fairness evaluation uses inferred gender labels as a proxy, standard practice when ground-truth demographic labels are unavailable~\cite{buolamwini2018gender}. We acknowledge that name-based gender inference typically achieves 70--85\% accuracy depending on the cultural composition of the dataset. The AIR value of $0.979$ should therefore be interpreted as an \textit{approximate} upper bound; with perfect demographic labels, the true AIR may differ. Uncertainty quantification via bootstrap resampling or using multiple inference algorithms would strengthen this result. Production deployments must use self-reported demographic data.
...
Looking forward, the most pressing extensions are hierarchical domain labeling to replace the coarse same-domain binary criterion; dynamic skill ontologies integrated with ESCO or O*NET to handle emerging technologies; domain-specific cross-encoder fine-tuning to unlock the full potential of joint-encoding approaches; and extension of FCR to intersectional multi-class protected attributes with Pareto-optimal fairness--relevance trade-offs. Each of these directions builds on a methodological foundation that this work provides: leakage-free evaluation, formal guarantees for fairness and counterfactual optimality, and an empirically grounded theory of signal complementarity in document ranking.
