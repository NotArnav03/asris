...
for _, row in tqdm(pairs.iterrows(), total=len(pairs)):
    ...
    resume_emb = resume_embeddings.get(row["resume_filename"])
...
# Median threshold with >= boundary semantics.  See
# ranking_utils.classify_at_percentile for the rationale.
